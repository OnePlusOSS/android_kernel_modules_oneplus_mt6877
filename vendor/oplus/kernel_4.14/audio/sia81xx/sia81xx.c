...
enum {
	SIA81XX_LEFT_CHANNEL = 0,
	SIA81XX_RIGHT_CHANNEL = 1,
};
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (0 == g_sia81xx->disable_pin) {
		ucontrol->value.integer.value[0] =
			(unsigned long)gpio_get_value(g_sia81xx->rst_pin);
	} else {
		ucontrol->value.integer.value[0] = 0;
	}
...
#else
	pr_debug("[debug][%s] %s: ucontrol = %ld, rst = %d \r\n",
		LOG_FLAG, __func__, ucontrol->value.integer.value[0], g_sia81xx->rst_pin);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (AUDIO_SCENE_NUM <= ucontrol->value.integer.value[0]) {
		g_sia81xx->scene = AUDIO_SCENE_PLAYBACK;
		pr_err("[  err][%s] %s: set audio scene val = %ld !!! \r\n",
			LOG_FLAG, __func__, ucontrol->value.integer.value[0]);
	} else {
		g_sia81xx->scene = ucontrol->value.integer.value[0];
	}

	if (0 == g_sia81xx->disable_pin) {
		if (SIA81XX_ENABLE_LEVEL == gpio_get_value(g_sia81xx->rst_pin)) {
			sia81xx_resume(g_sia81xx);
		}
	}
#endif /*CONFIG_SND_SOC_OPLUS_PA_MANAGER*/
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if (0 != sia81xx_regmap_read(g_sia81xx->regmap, addr, 1, &val)) {
		pr_debug("[debug][%s] %s: err regmap_read \r\n",
			LOG_FLAG, __func__);
	} else {
		for (i = 0;i < vol_length;i++) {
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!rstpin_is_on) {
		pr_debug("[debug][%s] %s: rstpin is not on ,can not set boost\r\n",LOG_FLAG, __func__);
		return 0;
	}
	if (index > 5) {
		pr_err("[err][%s] %s: index = %ld, rst = %d ucontrol_value is Invalid parameter\r\n",
			LOG_FLAG, __func__, index, g_sia81xx->rst_pin);
		return 0;
	}
#endif /* OPLUS_BUG_COMPATIBILITY */
...
#else
	if (0 != sia81xx_regmap_write(g_sia81xx->regmap, addr, 1, voltage+index)) {
		pr_err("[err][%s] %s: regmap_write \r\n",
			LOG_FLAG, __func__);
	}
#endif /* OPLUS_BUG_COMPATIBILITY */
...
{
	sia81xx_pa_enable(enable, mode, SIA81XX_LEFT_CHANNEL);
}

static void sia81xx_pa_enable_r(int enable, int mode)
{
	sia81xx_pa_enable(enable, mode, SIA81XX_RIGHT_CHANNEL);
}
	...
	const struct i2c_device_id *id)
{
	...
	if (((sia81xx_regmap_check_chip_id(sia81xx->regmap, sia81xx->chip_type) == 0)
		...
		speaker_device = kzalloc(sizeof(struct oplus_speaker_device), GFP_KERNEL);
                ...
                if((chip_type == CHIP_TYPE_SIA8102) || (chip_type == CHIP_TYPE_SIA8100)){
                        // there is AW87xxx, we need the speaker protection algorithm
                        if ((sia81xx->channel_num == 0) && (speaker_device != NULL)) {
                                speaker_device->speaker_protection_set = sia81xx_pa_enable_l;
                        } else if ((sia81xx->channel_num == 1) && (speaker_device != NULL)) {
		...
		if ((sia81xx->channel_num == 0) && (speaker_device != NULL)) {
			...
			speaker_device->speaker_mute_set = sia81xx_power_set;
			speaker_device->speaker_mute_get = sia81xx_power_get;
			oplus_pa_sia_node = oplus_speaker_pa_register(speaker_device);
			pr_info("%s():,L_SPK ,oplus_register end\r\n", LOG_FLAG, __func__);
		} else if ((sia81xx->channel_num == 1) && (speaker_device != NULL)) {
			speaker_device->chipset = MFR_SI;
			...
			speaker_device->speaker_mute_get = sia81xx_power_get;
			oplus_pa_sia_node = oplus_speaker_pa_register(speaker_device);
			pr_info("%s():R_SPK,oplus_register end\r\n", LOG_FLAG, __func__);
		} else {
		...
		if (speaker_device == NULL) {
			speaker_device = kzalloc(sizeof(struct oplus_speaker_device), GFP_KERNEL);
			if (speaker_device != NULL ) {
				speaker_device->type = sia81xx->channel_num + 1;
				is_new_speaker_device = true;
			} else {
		...
		if (is_new_speaker_device) {
			oplus_pa_sia_node = oplus_speaker_pa_register(speaker_device);
		}
		pr_info("%s():for aw87xxx,oplus_register end\r\n", LOG_FLAG, __func__);
	}
